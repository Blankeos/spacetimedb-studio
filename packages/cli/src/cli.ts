...
function debugPaths(): void {
  ...
  if (isCompiled) {
    ...
    console.log(chalk.gray(`  cliDistPath server exists: ${existsSync(join(cliDistPath, "dist/server/index.mjs"))}`))
    console.log(chalk.gray(`  appsStudioPath: ${appsStudioPath}`))
    console.log(chalk.gray(`  appsStudioPath server exists: ${existsSync(join(appsStudioPath, "dist/server/index.mjs"))}`))
  } else {
    const devPath = resolve(baseDir, "../../../apps/studio")
    console.log(chalk.gray(`  devPath: ${devPath}`))
    console.log(chalk.gray(`  devServerPath exists: ${existsSync(join(devPath, "dist/server/index.mjs"))}`))
  }
}
...
program
  ...
  .action(async (database: string | undefined, options: { db?: string; port: string; host: string; dev?: boolean; debug?: boolean }) => {
    ...
    if (validation.tables !== undefined) {
      console.log(chalk.dim(`    ${validation.tables} table(s), ${validation.reducers} reducer(s)`))
    }
  ...
  return new Promise((resolve) => {
    ...
    child.on("close", (code) => {
      if (code !== 0) {
        ...
        resolve({ 
          valid: false, 
          error: errorMsg.includes("not find") || errorMsg.includes("failed to find") || errorMsg.includes("Connection refused")
